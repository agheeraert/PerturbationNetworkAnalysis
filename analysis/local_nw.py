...
if __name__ =='__main__':
    for cutoff in range(3, 10):
        ...
        for u, v in nx.compose(apo_net, prfar_net).edges():
            if (u,v) in apo_net.edges() and (u, v) in prfar_net.edges():
                new_weight = delta_nw(prfar_net, apo_net, u)
            elif (u,v) in apo_net.edges() and (u,v) not in prfar_net.edges():
                new_weight = -nw(apo_net, u)-nw(apo_net,v)
            else:
                new_weight = nw(prfar_net, u)+nw(prfar_net,v)
            print(new_weight)
            if new_weight > 0:
                new_net.add_edge(u, v, color='r', weight=abs(new_weight))
            elif new_weight < 0:
                new_net.add_edge(u, v, color='g', weight=abs(new_weight))

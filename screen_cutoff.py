...
for node in L_nets_apo[0].nodes():
    ...
    ax1.plot(L_cutoffs, L_degrees_apo, marker='x', c='b', label='degree in apo, R^2= '+r2(L_cutoffs, L_degrees_apo))
    ax1.plot(L_cutoffs, L_degrees_bound, c='r', marker='+', label='degree in PRFAR-bound, R^2= '+r2(L_cutoffs, L_degrees_bound))
    f.suptitle("Degree distribution for node " + node)
    ax1.set_xlabel('Cutoff (in Å)')
    ax1.set_ylabel('Degree of the node')
    ax1.legend(loc='upper left')
    plt.savefig(os.path.join(args.output, node+'_degree.pdf'))
    f = plt.figure()
    ax2 = f.add_subplot(111)
    ax2.plot(L_cutoffs, L_weights_apo, c='b', marker='x', label='weight in apo, R^2= '+r2(L_cutoffs, L_weights_apo))
    ax2.plot(L_cutoffs, L_weights_bound, c='r', marker='+', label='weight in PRFAR-bound, R^2= '+r2(L_cutoffs, L_weights_bound))
    ax2.set_xlabel('Cutoff (in Å)')

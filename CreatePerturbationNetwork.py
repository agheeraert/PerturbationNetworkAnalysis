...
class CreatePerturbationNetwork(CreateNetwork):
    ...
    def draw_perturbation(self, threshold, output, rearrange=None, neighbors=True, chains=True, save=None):
        net = self.perturbation(threshold)
        ...
        if isinstance(save, str):
            nx.write_gpickle(net, join(output_folder, save))        
        if neighbors:
            if 'neighbors' not in os.listdir(output_folder):    
                os.mkdir(join(output_folder, 'neighbors'))
            if chains:
                if '4D' not in os.listdir(output_folder):    
                    os.mkdir(join(output_folder, '4D'))
            for node in net.nodes():
                subG = nx.Graph()
                ...
                plt.savefig(join(output_folder, 'neighbors', node+'.pdf'))
                if fourD and chains:
                    plt.savefig(join(output_folder, '4D', node+'.pdf'))
...
if __name__ == '__main__':
    ...
    CreatePerturbationNetwork(path1='/home/aria/PerturbationNetworkAnalysis/data/frame1000/frame_1000_apo.pdb', path2='/home/aria/PerturbationNetworkAnalysis/data/frame1000/frame_1000_prfar.pdb').draw_perturbation(threshold=40, output='/home/aria/PerturbationNetworkAnalysis/data/test.pdf', rearrange=('H', 'F'), save='test.p')
...
def plot_avg(M, name):
    ...
    for thresh in range(1,12,2):
        color = next(colors)
        plt.plot(range(3,8), M.transpose()[thresh//2], color=color)
        ...
        plt.xlabel('Cutoff shell (in $\AA)$')
        plt.xticks(range(3,8), [str(i)+'-'+str(i+2) for i in range(3,8)])
        plt.ylabel('Average '+name.split('.')[0].lower()+' per node in the perturbation network')
    plt.savefig(jn(args.o, name))


if __name__ =='__main__':
    W, K, WK = np.zeros((5, 6)), np.zeros((5, 6)), np.zeros((5, 6))
    for cutoff in range(3, 8):
        folder = jn(args.f, str(cutoff)+'-'+str(cutoff+2))
        for thresh in range(1, 12, 2): 
            try:
